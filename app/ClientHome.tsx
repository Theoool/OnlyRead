...
import { db, swapBookId } from "@/lib/db";
import { useLiveQuery } from "dexie-react-hooks";
import { toast } from "sonner";
import { saveArticle } from "@/app/actions/article";
import { importUrl, importFile } from "@/app/actions/import";
...
export default function ClientHome({ initialArticles, initialCollections }: ClientHomeProps) {
  ...
  // Extract articles from response or use initial data
  const articles = articlesData.articles?.length > 0 ? articlesData.articles : (initialArticles || []);
  
  // Use collections from react-query or initial props
  ...
  const sortedCollections = useMemo(() => {
    const remote = effectiveCollections.map((c: any) => ({ ...c, isLocal: false }));
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const performBackgroundUpload = async (localId?: string) => {
         try {
             ...
             // UNIFICATION: We no longer rely on local ID swapping for EPUB storage
             // Instead, we just refresh the collections list. 
             // Ideally we should pre-fetch the articles here to make it instant.
             ...
             if (data.data.collection) {
                // If it's a book/collection, go to collections view
      ...
      // UNIFICATION: Always use Serial Mode (Upload -> Process -> Read)
      // This ensures we always read the processed Markdown and vectors match content.
      setLoading(true);
  ...
  // NOTE: saveArticle function name conflict, so using saveArticleAction
  async function handleSaveArticle(article: Article) {
    try {
      await saveArticle(article);
      // await refetchArticles(); // Already handled in saveArticle
    } catch (error) {
